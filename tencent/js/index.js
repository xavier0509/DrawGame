...
function pageInit(){
	$.ajax({
		type: "get",
		async: true,
		url: "https://beta.restful.lottery.coocaatv.com/v1/lottery/indep/active/"+activeId,
		dataType: "jsonp",
		jsonp: "callback",
...
function showAwardList(){
	...
	$.ajax({
		type: "get",
		async: true,
		url: "https://beta.restful.lottery.coocaatv.com/v1/lottery/indep/awardList/"+activeId,
		data: {"size":"20"},
		dataType: "jsonp",
		jsonp: "callback",
		success: function(data) {
...
function rotateStart(){
	...
	var rotateFn = function(awards, angles, txt, typeid, lotteryAwardMemberId,awardId) { //awards:奖项，angle:奖项对应的角度
		...
		$('#rotate').rotate({
			...
			duration: 3000,
			callback: function() {
				document.getElementById("turntable_1").removeAttribute("disabled");
				//这里需要传递几个用得到的参数过去：奖品名称 图片url地址
				console.log("got the winner award" + txt + angles + awards + "---" + typeid + "---" + lotteryAwardMemberId + "--------" );
				//区分实体奖、虚体奖、谢谢参与
				//113---年卡，113月卡，109 7天卡，110谢谢
				if (awardId == '124') {
					dialogShow1("VIPMasking");
					exceptbutton("activateNow","");
					document.getElementById("toast-img-2-2").src = app.rel_html_imgpath(__uri("../images/124.png"));
				...
				// }
					...
					dialogShow1("VIPMasking");
					exceptbutton("activateNow","");
					document.getElementById("toast-img-2-2").src = app.rel_html_imgpath(__uri("../images/126.png"));
					document.getElementById("activateNow").onclick = function(){vipActive(lotteryAwardMemberId,awardId);}
				}
				else if (awardId != '125') {
					...
					exceptbutton("activateNow","");
					document.getElementById("goodLuckName").innerHTML = "恭喜您抽中【"+txt+"】";
	...
	$.ajax({
		type: "get",
		async: true,
		url: "https://beta.restful.lottery.coocaatv.com/v1/lottery/indep/lottery/"+activeId+"/" + mac + "/" + accesstoken,
		dataType: "jsonp",
		jsonp: "callback",
		success: function(data) {
			console.log("seccess...");
...
function vipActive(listId,awardId){
	...
	$.ajax({
		type: "get",
		async: true,
		url: "https://beta.restful.lottery.coocaatv.com/v1/lottery/indep/"+listId+"/" +awardId+"/"+activeId+"/"+ mac + "/" + accesstoken,
		dataType: "jsonp",
		jsonp: "callback",
		success: function(data) {
...
function showMyAward(){
	...
	if (loginstatus == "false") {experienceonclick();}
	else{
		...
		$.ajax({
			type: "get",
			async: true,
			url: "https://beta.restful.lottery.coocaatv.com/v1/lottery/indep/myAwards/"+activeId+"/"+accesstoken,
			dataType: "jsonp",
			jsonp: "callback",
			...
			success: function(data) {
				...
				for (var i = 0; i < _MyAwardsBeanlength; i++) {
					...
					_Number = i;
					if (_AwardId[i] == "122") {
						var awardImg = app.rel_html_imgpath(__uri('../images/122.png'));
					}
					if (_AwardId[i] == "123") {
						var awardImg = app.rel_html_imgpath(__uri('../images/123.png'));
					}
					if (_AwardId[i] == "124") {
						var awardImg = app.rel_html_imgpath(__uri('../images/124.png'));
					}
					if (_AwardId[i] == "125") {
						var awardImg = app.rel_html_imgpath(__uri('../images/125.png'));
					}
					if (_AwardId[i] == "126") {
						var awardImg = app.rel_html_imgpath(__uri('../images/126.png'));
					}
					if (_AwardId[i] == "127") {
						var awardImg = app.rel_html_imgpath(__uri('../images/127.png'));
					}
					if (_AwardId[i] == "128") {
						var awardImg = app.rel_html_imgpath(__uri('../images/128.png'));
					}
					if (_AwardId[i] == "129") {
						var awardImg = app.rel_html_imgpath(__uri('../images/129.png'));
					}
					if (_AwardId[i] == "130") {
						var awardImg = app.rel_html_imgpath(__uri('../images/130.png'));
					}
					if (_AwardId[i] == "132") {
						var awardImg = app.rel_html_imgpath(__uri('../images/132.png'));
					}
...
function generateQRCode(url) {
    ...
    document.getElementById("qcode").innerHTML="";
    $("#qcode").qrcode({ 
...
function generateQRCode2(url) {
    console.log("create img--------------");
    document.getElementById("qcode2").innerHTML="";
    $("#qcode2").qrcode({ 
        render: "canvas", // 渲染方式有table方式（IE兼容）和canvas方式
        width: 170, //宽度 
        height:170, //高度 
